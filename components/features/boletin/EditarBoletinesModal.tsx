// components/features/boletin/EditarBoletinesModal.tsx
"use client";
...
import { SortableItem } from "@/components/features/boletin/SortableItem";
import { AudioFromAPI } from "@/types/api";

interface EditarBoletinesModalProps {
  open: boolean;
...
export function EditarBoletinesModal({
  open,
  onOpenChange,
  selectedBoletines,
  currentStartTime,
  ...
  const handleOpenChange = (newOpen: boolean) => {
    if (newOpen) {
      ...
      const date = new Date(currentStartTime);
      setStartTime(normalizeMinutes(date));
      setErrors({});
  ...
  const validateForm = (): boolean => {
    ...
    if (items.length !== 30) {
      newErrors.items = `Debes tener exactamente 30 informaciones. Actualmente tienes ${items.length}`;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-hidden flex flex-col sm:max-w-[95vw]">
        <DialogHeader>
          <DialogTitle>Editar boletín #{boletinId}</DialogTitle>
          <DialogDescription>
            Arrastra las tarjetas para reordenar las informaciones.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="gap-2">
          ...
          >
            {isSubmitting ? (
              ...
              `Actualizar boletín (${items.length}/30)`
            )}
          </Button>
        </DialogFooter>
      </DialogContent>
